...
export class BracketRangesProvider extends BetterFoldingRangeProvider {
  ...
  private getCollapsedText(bracketsRange: BracketsRange, document: TextDocument, shallow = false): string {
    ...
    if (showFoldedBodyLinesCount) {
      let fullTextLength = document.lineCount
      collapsedText = this.getFoldedLinesCountCollapsedText(bracketsRange, fullTextLength);
    }
  ...
  private getFoldedLinesCountCollapsedText(bracketsRange: BracketsRange, totalLines: number): string {
    ...
    let percentage = 0;
    if (linesCount > 0) {
      percentage = (linesCount/ totalLines)*100
    }
    const line = linesCount === 1 ? "line" : "lines";
    return ` ⋯ ${linesCount} ${line} (${percentage.toFixed(2)}%) ⋯ `;
  }
